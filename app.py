...
st.set_page_config(
    page_title="Kickboxing Analysis", layout="wide", initial_sidebar_state="collapsed"
)
floor_selector = components.declare_component(
    "floor_selector", path=str(FLOOR_SELECTOR_DIR)
)


def select_floor_corners(
    image: Image.Image, points: list[tuple[float, float]], key: str
...
def analyse_session(video, person: kba.Person) -> None:
    ...
    with tempfile.NamedTemporaryFile(
        suffix=Path(video.name).suffix, delete=False
    ) as file:
    ...
    # Stats
    striking_stats = kba.StrikingStatsCalculator(
        result.person_state, result.strike_detections, STRIKE_CONFIG
    ).calculate_striking_stats()
    guard_stats = kba.DefenseStatsCalculator(
        result.person_state, result.guard_detections
    ).calculate_guard_stats()
...
def render_setup() -> None:
    ...
    with st.form("session_setup"):
        video = st.file_uploader(
            "Training clip",
            type=["mp4", "mov", "avi"],
            help="One fighter in frame; shadowboxing footage works best.",
        )
        ...
        wingspan = st.number_input(
            "Wingspan (m)", min_value=1.0, max_value=2.5, value=None
        )
...
def render_calibration(compact: bool = False) -> kba.FootworkAnalyser | None:
    if compact:
        st.caption(
            "Adjust corners or floor dimensions. Changes replace the current footwork output."
        )
    else:
        st.markdown(
            "<p class='workspace-kicker'>Floor tracer</p>", unsafe_allow_html=True
        )
        st.markdown(
            "<p class='workspace-copy'>Set the four floor corners, then drag a marker to refine the projection.</p>",
    ...
    display_frame = Image.fromarray(frame).resize(
    ...
    display_points = [
        (
            point[0] * DISPLAY_WIDTH / frame.shape[1],
            point[1] * display_height / frame.shape[0],
        )
        for point in st.session_state.floor_points
    ]

    value = select_floor_corners(
        display_frame, display_points, f"floor-selector-{revision}"
    )
    ...
    if isinstance(value, list) and len(value) <= 4:
        selected_points = [
            (
                float(point[0]) * frame.shape[1] / DISPLAY_WIDTH,
                float(point[1]) * frame.shape[0] / display_height,
            )
            for point in value
    ...
    st.caption(
        "Floor dimensions (optional). Enter both side lengths for results in metres; leave them at zero for a unit-square projection."
    )
    left_col, right_col, width_col = st.columns(3)

    with left_col:
        left_length = st.number_input(
            "Left side (m)", min_value=0.0, step=0.1, key="floor_length1"
        )
    with right_col:
        right_length = st.number_input(
            "Right side (m)", min_value=0.0, step=0.1, key="floor_length2"
        )
    with width_col:
        width_m = st.number_input(
            "Front edge / width (m)", min_value=0.0, step=0.1, key="floor_width"
        )
...
def render_striking(stats: kba.StrikingStats) -> None:
    ...
    thrown = [
        name
        for name, speeds in stats.strike_speeds.items()
    ...
    for tab, name in zip(st.tabs(labels), thrown):
        chart = pd.DataFrame(
            {
                "Time (s)": stats.strike_times_s[name],
                "Speed (m/s)": stats.strike_speeds[name],
            }
        )
...
def create_footwork_output(analyser: kba.FootworkAnalyser) -> None:
    with st.spinner("Rendering footwork visualisations..."):
        ...
        st.session_state["footwork_output"] = {
            ...
            "width_unit": "m"
            if analyser.projector.floor_edge_lengths is not None
            else "sq",
            "figure": figure,
...
def render_footwork(analyser: kba.FootworkAnalyser) -> None:
    ...
    distance = (
        stats.distance_travelled_cumsum[-1]
        if stats.distance_travelled_cumsum.size
        else 0.0
    )
    totals = st.columns(4)
...
def render_review(analyser: kba.FootworkAnalyser) -> None:
    ...
    video_col, metrics_col = st.columns((1.65, 0.8))
    with video_col:
...
def render_session_panel() -> None:
    ...
    if st.button(
        "New session", type="secondary", use_container_width=True, key="new_session"
    ):
        for key in (
            "result",
            "stats",
