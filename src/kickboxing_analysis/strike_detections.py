...
class StrikeDetections:
    ...
    def expanded(self, before: int = 0, after: int = 30) -> StrikeDetections:
        """
        ...
        for row_expanded, row in zip(expanded, self.mask):
            for frame_idx in np.flatnonzero(row):
                start = max(0, frame_idx - before)
                end = min(self.n_frames, frame_idx + after + 1)
                row_expanded[start:end] = True

        return StrikeDetections(self.strikes, expanded)
